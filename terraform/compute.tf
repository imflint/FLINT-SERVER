...
resource "aws_instance" "admin" {
  ...
  key_name                    = var.ec2_key_name != "" ? var.ec2_key_name : null
  associate_public_ip_address = true
  disable_api_termination     = var.admin_ec2_disable_api_termination
  user_data_replace_on_change = true
  ...
  root_block_device {
    volume_size           = var.admin_ec2_volume_size
    volume_type           = var.ec2_volume_type
    encrypted             = true
    delete_on_termination = true
  }
...
resource "aws_eip" "admin" {
  count = var.admin_create_eip ? 1 : 0

  domain = "vpc"

  tags = {
    Name = "${local.name_prefix}-admin-api-eip"
  }
}
...
resource "aws_eip_association" "admin" {
  ...
  allocation_id = aws_eip.admin[0].id
